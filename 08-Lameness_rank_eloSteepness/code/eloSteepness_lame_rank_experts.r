...
# assign color of each cow based on their GS
gs_record3 <- gs_record2
colnames(gs_record3) <- c("id", "GS")
worker_expt_sum <- merge(worker_expt_sum, gs_record3)
num_colors <- 8
worker_expt_sum$color <- rev(magma(num_colors))[round(((worker_expt_sum$GS - 1)/4) * (num_colors-1)) + 1]

# Create the plot
elo_plot <- ggplot(worker_expt_sum, aes(x = expert_mean, y = worker_mean)) +
  geom_point(aes(color = I(color)), alpha = 0.9) +
  geom_ellipse(aes(x0 = (expert_q045 + expert_q955) / 2,
                   ...
                   angle = 0,
                   fill = I(color)), alpha = 0.8, color = NA)  +
  labs(
