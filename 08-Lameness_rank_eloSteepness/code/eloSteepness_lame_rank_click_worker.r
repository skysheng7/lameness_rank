...
correlation_change_df <- icc_change_worker_num_milestone(random_rounds = 10, max_worker_num = 14, 
                                ...
                                expert_col_name = "NV_DW_SB_KI_experts_mean",
...
icc_with_full_expert_sum <- avg_se(correlation_change_df, 
                                        ...
                                        by_var = c("num_of_crowd_worker"))
num_comparison_sum <- avg_se(correlation_change_df, 
                                   value_var = "num_of_comparison", 
                                   by_var = c("num_of_crowd_worker"))
subsample_sum <- merge(spearman_with_full_worker_sum, icc_with_full_expert_sum)
subsample_sum <- merge(subsample_sum, num_comparison_sum)
...
plot_A <- ggplot(correlation_change_df_sum, aes(x = num_of_crowd_worker, y = spearman_subsample_with_full_worker)) +
  geom_jitter(color = "dodgerblue", alpha = 0.5, size = 6, width = 0.1, height = 0) +
  geom_point(aes(y = spearman_subsample_with_full_worker_cor_mean), color = "midnightblue", alpha = 0.08, size = 17) +
  geom_errorbar(aes(
...
plot_B <- ggplot(correlation_change_df_sum, aes(x = num_of_crowd_worker, y = icc_subsample_with_full_expert)) +
  geom_jitter(color = "dodgerblue", alpha = 0.5, size = 6, width = 0.1, height = 0) +
  geom_point(aes(y = icc_subsample_with_full_expert_cor_mean), color = "midnightblue", alpha = 0.08, size = 17) +
  geom_errorbar(aes(
      x = num_of_crowd_worker,
...
plot_C <- ggplot(correlation_change_df_sum, aes(x = num_of_crowd_worker, y = num_of_comparison)) +
  geom_jitter(color = "dodgerblue", alpha = 0.5, size = 6, width = 0.1, height = 0) +
  geom_point(aes(y = num_of_comparison_cor_mean), color = "midnightblue", alpha = 0.08, size = 17) +
  geom_errorbar(aes(
    x = num_of_crowd_worker,
...
ggsave("../plots/combined_plot_vertical.png", plot = combined_plot, width = 15, height = 30, limitsize = FALSE)
